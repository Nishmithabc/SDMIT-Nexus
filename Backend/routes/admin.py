...
def send_password(req:SendPasswordRequest, db: Session = Depends(get_db)):
    ...
    email=req.email
    if not email.endswith("@sdmit.in"):
        raise HTTPException(status_code=403,detail="make sure you use sdmit email")
...
def remove_single_student(student_id: int,db: Session = Depends(get_db)):
...
def remove_students_from_group(group_data: dict , db: Session = Depends(get_db)):
    # Get the group
    ...
    deleted_count = db.query(Student).filter(Student.group_id == group.group_id).delete(synchronize_session=False)
    db.commit()
    
    return {"message": f"{deleted_count} students removed from group '{group_name}'"}
