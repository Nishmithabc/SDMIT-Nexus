...
export default function LecturerGroups() {
  ...
  if (newGroup.branch && newGroup.year) {
    ...
    try {
      ...
      toast({
        title: res.data.message || "Group Added",
        description: `${res.data.branch} - ${res.data.year}`,
      })

      // Re-fetch all groups to show updated list
      const updatedGroups = await api.get("/lecturer/groups")
      setGroups(updatedGroups.data)

      // Update local user cache as well
      const user = authService.getCurrentUser()
      if (user && Array.isArray(user.groups)) {
        if (!user.groups.includes(res.data.group_id)) {
          user.groups.push(res.data.group_id)
  ...
  const handleOpenGroup = (group: SelectedGroup) => {
    const id = group.group_id || (group as any)._id
  if (!id) {
    toast({
      title: "Invalid Group",
      description: "This group does not have a valid ID.",
      variant: "destructive",
    })
    return
  }
    navigate(`/lecturer-dashboard/${id}`, { state: { group } })
  }
