...
import { 
  User, Calendar, Building, LogOut, ArrowLeft, Users ,Trash2
} from "lucide-react"
...
useEffect(() => {
  const fetchGroups = async () => {
    if (!isStudent) {
      try {
        const res = await api.get("/lecturer-groups/groups/all")
        const map = {}
        res.data.forEach((g) => {
          map[g.id] = g.name
        })
    ...
    {user.groups && user.groups.length > 0 ? (
      <ul className="space-y-2">
        {user.groups.map((g, i) => (
          <li 
            key={i} 
            className="flex items-center justify-between p-2 bg-muted rounded shadow-sm text-sm font-medium"
          >
            <span>Group: {groupMap[g] || `ID: ${g}`}</span>
            <Button
              variant="destructive"
              size="icon"
              onClick={async () => {
                try {
                  await api.delete(`/lecturer-groups/group-leave/${g}`) // call leave API
                  toast({
                    title: "Left Group",
                    description: `You have successfully left ${groupMap[g] || `ID: ${g}`}`
                  })
                  // Remove group from local state
                  const updatedGroups = user.groups.filter((grp) => grp !== g)
