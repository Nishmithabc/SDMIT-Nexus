...
const handleDeleteMessage = (id: number) => {
  ...
  description: "Face verified successfully! Proceeding to sign the document.",
})
  ...
  setStatus("✅ Document successfully signed!");
  toast({
    title: "Document Signed Successfully",
    description: "Your document has been signed and recorded.",
  });
}
      } catch (err) {
        ...
        toast({
          title: "Error",
          description:err.response?.data?.detail ||"Failed to sign document.",
          variant: "destructive",
        });
      } finally {
        setModalDocId(null);
...
const intervalCheck = setInterval(() => {
  ...
  if (lastFrameData && lastFrameData !== currentFrame) {
    motionDetected = true;
  }
  lastFrameData = currentFrame;
  framesChecked++;

  if (!success && framesChecked > MAX_FRAMES) {
    if (motionDetected) {
      toast({
        title: "Liveness Failed",
        description: "Face movement detected but prompt not completed in time.",
        variant: "destructive",
      });
    } else {
      toast({
        title: "Liveness Failed",
        description: "No noticeable face activity detected. Please retry.",
        variant: "destructive",
      });
    }
    setModalDocId(null);
...
{confirmDeleteId && (
  ...
  // 1️⃣ Unsigned before signed
  if (aSigned !== bSigned) return aSigned ? 1 : -1;

  // 2️⃣ Non-expired before expired
  if (aExpired !== bExpired) return aExpired ? 1 : -1;

  // 3️⃣ Sooner deadlines before later ones
  return aDeadline - bDeadline;
})
    .map((doc) => {
      const now = new Date()
              ...
              >
                ...
                <div className="flex flex-col items-end space-y-2 min-w-max">
                  ...
                  {hasSigned ? (
                    ...
                    <Button
                        disabled={isExpired}
                        onClick={() => setPendingDocId(doc.document_id)}
                    >
      ...
      {pendingDocId && !modalDocId && (
  <div className="fixed inset-0 flex items-center justify-center bg-black/60 z-50">
    <div className="bg-white rounded-xl shadow-xl p-8 w-[400px] space-y-4 text-center">
      <h2 className="text-lg font-semibold text-gray-800">Disclaimer</h2>
      <p className="text-sm text-gray-600">
  By proceeding, you agree to use your camera for face verification. 
  Follow the prompts shown and make sure you’re in good lighting for a clear capture. 
  Use your own face — not your friend’s 😊. 
  Don’t worry, your image is processed securely and never stored after signing.
</p>
      <div className="flex justify-center gap-4 pt-2">
        <Button variant="secondary" onClick={() => setPendingDocId(null)}>
          Cancel
        </Button>
        <Button
          onClick={() => {
            setModalDocId(pendingDocId);
  ...
  <div className="fixed inset-0 flex items-center justify-center bg-black/70 z-50">
    <div className="min-h-screen bg-gradient-subtle rounded-2xl shadow-xl p-8 w-full max-w-3xl text-center space-y-6">
      ...
      <div className="flex justify-center gap-4 pt-4">
        <Button
  variant="secondary"
  disabled={status.includes("Liveness check passed") || status.includes("Document successfully signed")}
